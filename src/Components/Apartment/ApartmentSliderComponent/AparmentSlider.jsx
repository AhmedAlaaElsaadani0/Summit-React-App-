...
export default class AparmetSlider extends Component {
  render() {
    ...
    return <React.Fragment>
      <div id={"carouselExampleWhite"+uniqeNum} key={uniqeNum} className="carousel h-100 carousel-white hide slide">
        <div className="carousel-indicators">
          {
            
            this.props.flatImages.length >0? 
            this.props.flatImages.map((item, index) => {
              return <button  type="button" data-bs-target={"#carouselExampleWhite"+uniqeNum} data-bs-slide-to={index} style={{ width: "20px" }} className={index == 0 ? "active rounded-circle py-2 bg-primColor" : 'rounded-circle py-2 bg-primColor'} aria-current="true" aria-label={`Slide ${index + 1}`}></button>
            })
          :
        ...
        <div className="carousel-inner h-100">
        {
            ...
            this.props.flatImages.length >0? 
            this.props.flatImages.map((item, index) => {
              return <div  className={index == 0 ? "carousel-item h-100 active" : 'carousel-item h-100'} data-bs-interval={(index+1) *10000}>
